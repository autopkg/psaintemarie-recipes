...
#
# A URLDownloader-like processor for URLs behind Cloudflare's JS challenge,
# which blocks curl by TLS fingerprint. Apple's native network stack
# (NSURLSession) passes the challenge, so the download is performed via
# osascript/JXA, which needs no tooling beyond stock macOS.
#
...
class NSURLSessionDownloader(Processor):
    ...
    fingerprint. Output variables follow URLDownloader's common outputs."""
    ...
    input_variables = {
        ...
        "request_headers": {
            "required": False,
            "description": (
                "Optional dictionary of headers to include with the "
                "download request."
            ),
        },
        "request_timeout": {
    ...
    def get_request_headers(self):
        ...
        if not isinstance(request_headers, dict):
            raise ProcessorError(
                "request_headers must be a dictionary of header-name: value "
                "pairs"
            )
        return {
            str(key): str(value)
    ...
    def remove_temp_file(self, temp_pathname):
        try:
    ...
    def main(self):
        ...
        self.env["etag"] = ""
        self.env.pop("url_downloader_summary_result", None)
        ...
        try:
            try:
                ...
                raise ProcessorError(
                    f"Download of {self.env['url']} timed out"
                )
            finally:
            ...
            if status == 304:
                if not os.path.isfile(pathname):
                    raise ProcessorError(
                        "Server returned HTTP 304 but no cached file exists"
                    )
                ...
                self.output(
                    f"Item at URL is unchanged. Using existing {pathname}"
                )
                return
            ...
            if not os.path.isfile(temp_pathname) or not os.path.getsize(
                temp_pathname
            ):
                raise ProcessorError("Downloaded file is empty")

            os.rename(temp_pathname, pathname)
            temp_pathname_published = True
            os.chmod(pathname, 0o644)
            self.env["download_changed"] = True
            self.env["etag"] = response_headers.get("etag", "")
            self.env["last_modified"] = response_headers.get(
                "last-modified", ""
            )
            if self.env["etag"]:
                xattr.setxattr(
            ...
            if not temp_pathname_published:
                self.remove_temp_file(temp_pathname)
